...
void SteamCommon::Init()
{
	...
	if (!PM_Connect(MASTER_SERVER, 14030))
	{
		Addresses::Com_Error(ERR_FROM_STARTUP, "Could not connect to PM Server at " MASTER_SERVER);
	}
	...
	PMAsync<PMAuthenticateResult>* async = PM_CreateSessionWithCredentials(Global::Variables::Steam_Username, Global::Variables::Steam_Password, Global::Variables::Steam_Username);
	//PMAsync<PMAuthenticateResult>* async = PM_CreateSessionWithKey("1234567890abcdefghjklmnopqrstwxyz");
	result = async->Wait();
	...
	if (result->result != AuthenticateResultOK)
	{
		...
		case AuthenticateResultBadDetails:
			Addresses::Com_Error(ERR_FROM_STARTUP,  "Could not authenticate to PM server at " MASTER_SERVER " -- bad details | wrong username and password.");
			break;
		case AuthenticateResultAlreadyLoggedIn:
			Addresses::Com_Error(ERR_FROM_STARTUP,  "Could not authenticate to PM server at " MASTER_SERVER " -- already logged in.");
			break;
		case AuthenticateResultServiceUnavailable:
			Addresses::Com_Error(ERR_FROM_STARTUP,  "Could not authenticate to PM server at " MASTER_SERVER " -- service unavailable.");
			break;
		case AuthenticateResultBanned:
			Addresses::Com_Error(ERR_FROM_STARTUP,  "Could not authenticate to PM server at " MASTER_SERVER " -- banned.");
			break;
		case AuthenticateResultUnknown:
			Addresses::Com_Error(ERR_FROM_STARTUP,  "Could not authenticate to PM server at " MASTER_SERVER " -- unknown error.");
			break;
		}
	...
	PM_SetRichPresence("currentGame", "t6mp");
	PM_SetRichPresenceBody("Playing T6MP");
	PM_StoreRichPresence();

	PMAsync<PMGetOnlineFriensResult>* async2 = PM_GetOnlinePlayers("pt6");
